 ...
 * center. Opened in its own browser window (see ``lib/chat-window.ts``), it
 * holds one streaming, multi-turn session over the ``/api/chat/stream``
 * WebSocket. Claude reads freely and streams its answer token-by-token; writes,
 * shell commands and skill-runs each surface an inline approval card (with a
 * diff / command / skill preview) that David approves or declines before the
 * action runs.
 */
...
export function Chat() {
  ...
  function send() {
    ...
    const ws = wsRef.current
    if (!text || !ws || ws.readyState !== WebSocket.OPEN || conn === "streaming") return
    setMessages((prev) => [
      ...prev,
      { id: newId(), role: "user", text, thinking: "", tools: [], streaming: false },
  ...
  return (
    <div className="flex h-screen flex-col bg-bg text-text">
      <ChatHeader conn={conn} sessionId={sessionId} page={page} onReconnect={connect} />
  ...
  return (
    <header className="flex h-12 shrink-0 items-center justify-between border-b border-border bg-bg-panel px-4">
      <div className="flex items-center gap-2">
        <span className="text-sm font-semibold tracking-tight lowercase">command center · chat</span>
